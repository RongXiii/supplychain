...
from data.data_source import DataSourceFactory, DataTransformer, DataValidator
from data.data_warehouse import DataWarehouse
from mlops.real_time_processor import RealTimeDataProcessor, RealTimeForecastUpdater
...
class ReplenishmentSystem:
    ...
    def __init__(self):
        ...
        self.data_warehouse = DataWarehouse()  # 新增数据仓库实例
        self.models = {}
...
def generate_sample_data():
    ...
    """
    # 使用模拟数据源生成数据
    data_source = DataSourceFactory.create_data_source('simulated')
    
    # 获取所有数据
    items = data_source.get_items()
    locations = data_source.get_locations()
    suppliers = data_source.get_suppliers()
    inventory_daily = data_source.get_inventory_daily()
    purchase_orders = data_source.get_purchase_orders()
    forecast_output = data_source.get_forecast_output()
    optimal_plan = data_source.get_optimal_plan()
    ...
    print("样本数据已保存到data目录")
    
    # 生成产品历史数据
    sample_data = {}
    for item_id in inventory_daily['item_id'].unique():
        # 获取该产品的历史需求数据
        item_df = inventory_daily[inventory_daily['item_id'] == item_id].copy()
...
def main():
    ...
    # 配置数据源 - 可以根据需要切换为 'database' 或 'api'
    data_source_type = 'csv'  # 可选值: 'csv', 'database', 'api', 'simulated'
    
    # 配置数据源参数
    data_source_config = {
        'data_dir': './data'  # CSV数据源的目录
        # 数据库数据源示例配置:
        # 'connection_string': 'mysql+mysqlconnector://username:password@localhost:3306/supplychain'
        # API数据源示例配置:
    ...
    # 创建数据源实例
    print(f"📊 正在从 {data_source_type} 数据源加载数据...")
    data_source = DataSourceFactory.create_data_source(data_source_type, data_source_config)
    ...
    # 将数据整合为simulated_tables格式，以便后续处理
    simulated_tables = {
        'items': items,
        'locations': locations,
        'suppliers': suppliers,
    ...
    print("\n\n🚀 第3步：初始化数据仓库")
    print("-" * 40)
    print("📦 正在将数据加载到统一数据仓库...")
    
    # 向数据仓库注册数据源
    system.data_warehouse.register_data_source('primary', data_source)
    
    # 将数据加载到数据仓库
    for table_name, df in simulated_tables.items():
        system.data_warehouse.update_data(table_name, df, update_type='replace')
    
    # 展示数据仓库中的数据表信息
    print("\n✅ 数据仓库中的数据表详情：")
    for table_name in simulated_tables.keys():
        try:
            df = system.data_warehouse.get_data(table_name)
    ...
    # 获取数据血缘信息
    lineage = system.data_warehouse.get_data_lineage()
    print(f"\n✅ 生成了 {len(lineage)} 条数据血缘记录")
    ...
    for table_name in simulated_tables.keys():
        try:
            metrics = system.data_warehouse.get_data_quality_metrics(table_name)
            print(f"\n  • {table_name}:")
    ...
    # 创建实时数据处理器
    real_time_processor = RealTimeDataProcessor(system.data_warehouse)
    
    # 向数据仓库添加实时数据流
    system.data_warehouse.add_real_time_data_stream('inventory', real_time_processor)
    
    # 处理实时数据
    print("\n📥 正在处理实时数据...")
    system.data_warehouse.process_real_time_data()
    ...
    # 创建实时预测更新器
    real_time_forecast_updater = RealTimeForecastUpdater(system)
    
    # 更新预测
    real_time_forecast_updater.update_forecasts()
    
    # 展示更新后的预测数据
    try:
        forecast_df = system.data_warehouse.get_data('forecast_output')
        print(f"\n✅ 预测数据已更新：")
        print(f"  • 预测表总记录数: {len(forecast_df)}")
        # 展示最新的5条预测记录
        print(f"  • 最新5条预测记录：")
        latest_forecasts = forecast_df.sort_values('date', ascending=False).head(5)
        for _, row in latest_forecasts.iterrows():
            print(f"    - {row['date']} | {row['sku']} | 预测需求: {row['forecasted_demand']:.2f} | 模型: {row['model_used']}")
    except Exception as e:
        print(f"\n❌ 更新预测失败: {str(e)}")
    ...
    # 验证数据完整性
    is_valid, errors = DataValidator.validate_all(
        items, locations, suppliers, inventory_daily, purchase_orders, forecast_output, optimal_plan
    )
